...
def get_pool_state(contract: ContractInfo, is_mainnet: bool = True) -> PoolState:
    ...
    if total_microtokens == 0:
        ...
        total_cost = total_tokens * asset_price if is_mainnet else 1
...
def recalculate_reward(
    pool: PoolState, current_block: int, staked: int, reward: int, reward_per_token_paid: int
) -> int:
    ...
    for pool_id, state in local_states.items():
        try:
            ...
            if is_mainnet:
                ...
                reward_usd += (
                    reward
                    * pool_state.total_algo_rewards
                    // pool_state.total_rewards
                    * get_algo_price()
                    / MICROALGOS_IN_ALGO
                )
