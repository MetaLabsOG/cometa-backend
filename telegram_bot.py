...
from bot.db.users import bot_users, create_user, get_user_by_tg
from bot.env import FEEDBACK_COMMAND, MESSAGE_ALL_COMMAND, SUPPORT_COMMAND, bot_settings
from bot.formatting import format_user_pool
from bot.log import LOG_DATE_FORMAT, LOG_FORMAT
...
async def start(update: Update, context: CallbackContext):
    await update.message.reply_html(
        f"🤖 I'm glad to see you, {update.message.from_user.name}, let's be friends!👀\n\n"
...
async def check_registration(update: Update) -> Optional[BotUser]:
    ...
    if user is None:
        await update.message.reply_html(
            "🤖 Please register first with:\n"
            "<code>/register YOUR_ALGO_ADDRESS</code>\n\n"
            "To know more about me please use:\n"
            "/help"
        )
        return None
    return user
...
async def show_pools(update: Update, context: CallbackContext):
    ...
    if pools:
        ...
        reply_text = (
            "🤖 You don't have any pools, that's strange..."
            "\n\n"
...
async def message_all(update: Update, context: CallbackContext):
    tg_user = update.message.from_user
    if tg_user.id not in bot_settings.telegram_admin_ids:
        logging.warning("Rejected message_all command from Telegram user %s", tg_user.id)
...
async def change_address(update: Update, context: CallbackContext):
    if await check_registration(update) is None:
        return
...
async def show_help(update: Update, context: CallbackContext):
    text = (
        f"🤖 <i>Hello, {update.message.from_user.name}, it is a pleasure to assist you!</i>☺️"
        f"\n\n"
