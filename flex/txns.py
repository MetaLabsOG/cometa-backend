...
from flex.blockchain import algod_client, AssetInfo, cometa_public_key, cometa_private_key
...
def get_tx_info_with_wait(txid: str, tx_type: TxType, timeout_rounds: int = TX_WAIT_ROUNDS) -> TxInfo:
    try:
        ...
        logger.error(f'Transaction with id = {txid} is not found: {e}', exc_info=True)
        raise MetaError(f'Transaction with id = {txid} is not found: {e}')
...
def send_asset_micros_with_wait(
    ...
    logger.debug(f'Sent {asset_info.micros_to_amount(amount_micros)} {asset_info.unit_name} to {address} with txid: {txid}!')
    return tx_info
