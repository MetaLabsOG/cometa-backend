import logging

from flex import db
from flex.blockchain.info import ALGO_ASSET
from flex.db.model.liquidity_pools import LpState
from flex.db.model.priced import AssetPrice

logger = logging.getLogger(__name__)


async def calculate_price_algo_from_tiny_algo_pool(lp_state: LpState) -> float:
    if not lp_state.is_algo_pool:
        raise ValueError(f'Not an ALGO pool: {lp_state}')

    if lp_state.asset2_id != ALGO_ASSET.id:
        raise ValueError(f'ALGO pool id2 != ALGO: {lp_state}')

    if lp_state.asset1_reserve_micros == 0:
        return 0

    return lp_state.asset2_reserve / lp_state.asset1_reserve


async def update_tinyman_algo_lp_state_and_prices(lp_state: LpState, algo_price_usd: float) -> AssetPrice:
    if not lp_state.is_algo_pool:
        raise ValueError(f'Not an ALGO pool: {lp_state}')
    logger.debug(f'Updating "{lp_state.asset1_id}" price with Tiny ALGO pool {lp_state.id}')

    # TODO: remove test log
    if lp_state.asset1_id == 0:
        logger.error(f'\n\n\nALGO pool with asset1_id = 0: {lp_state}\n\n')
        raise ValueError(f'ALGO pool with asset1_id = 0: {lp_state}')

    # TODO: should I update LP token price in DB ?
    lp_state.token_price_algo = lp_state.asset2_reserve * 2 / lp_state.total_tokens
    db.lp_states.update(lp_state)

    asset_price_algo = await calculate_price_algo_from_tiny_algo_pool(lp_state)
    asset_price_usd = asset_price_algo * algo_price_usd

    asset_price = db.asset_prices.get_one(id=lp_state.asset1_id)
    if asset_price is None:
        asset_price = AssetPrice(
            id=lp_state.asset1_id,
            price_algo=asset_price_algo,
            price_usd=asset_price_usd,
            last_update_round=lp_state.last_updated_round,
            tinyman_algo_pool_id=lp_state.id
        )
        db.asset_prices.create(asset_price)
    else:
        asset_price.price_algo = asset_price_algo
        asset_price.price_usd = asset_price_usd
        asset_price.last_update_round = lp_state.last_updated_round
        db.asset_prices.update(asset_price)

    logger.debug(f'Updated "{lp_state.asset1_id}" price with Tiny ALGO pool {lp_state.id}: algo = {asset_price_algo}, usd = {asset_price_usd}')

    return asset_price
